...
# Fitness traits
coefs = readRDS(file = "Intermediate/Fitness/LinearFits.rds")
...
fitness = fitness %>%
  ...
  dplyr::select(Trait, ID,
                ...
                Lp26_C, Lp26_HT, Lp26_PS, Lp26_D, Lp26_HTD, Lp26_PSD)
...
fitness = map(c("An1", "Bay0", "Col0", "Lp26", "Average"), function(x) {
  out = select(fitness, Trait, ID, contains(x))
  names(out) = c("Trait", "ID", "C","HT", "PS", "D", "HTD", "PSD")
  mutate(out, Genotype = x)}) %>% 
  do.call("rbind", .)

# Reshape to longest format
fitness = pivot_longer(fitness, c(-Trait, -Genotype, -ID), 
                       names_to = "Effect", values_to = "Value")

# Reshape to wider format separating traits
fitness = pivot_wider(fitness, names_from = "Trait", values_from = "Value")


# Save results ------------------------------------------------------------

saveRDS(fitness, file = "Intermediate/Fitness/DataForCorrelations.rds")

# Calculate mean and stdev of each trait ----------------------------------

fitness = readRDS("Intermediate/Fitness/DataForCorrelations.rds")

sum_fitness = group_by(fitness, Genotype, Effect) %>%
  summarise(muFT = median(FT), seFT = sd(FT),
            muYield = median(Yield), seYield = sd(Yield),
            muLN = median(LN), seLN = sd(LN),
            muPhyllochron = median(Phyllochron), sePhyllochron = sd(Phyllochron))
